...
const Leaderboard = () => {
  ...
  const filteredLeaderboard = leaderboard.filter((entry) =>
    `${entry.team?.name || ''} ${entry.score}`.toLowerCase().includes(filter.toLowerCase()),
  );
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <form className="row g-3 align-items-center mb-4" onSubmit={(e) => e.preventDefault()}>
            <div className="col-auto flex-grow-1">
              ...
              <input
                ...
                placeholder="Filter by team or score"
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
              />
            </div>
            <div className="col-auto">
              <button type="button" className="btn btn-success" onClick={() => setFilter('')}>
                Clear
              </button>
            </div>
          </form>
          ...
          {loading ? (
            ...
            <p>No leaderboard entries match your search.</p>
          ) : (
